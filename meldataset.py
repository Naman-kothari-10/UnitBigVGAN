...
class CodeDataset(torch.utils.data.Dataset):
    def __init__(self, audio_files, unit_file_path, utt2lang_filepath, lang_ids_json, spk_emb_scp_filepath, segment_size, n_fft, num_mels,
                 ...
                 split=True, shuffle=False, fmax_loss=None, is_multispkr=False, is_f0=False, is_multilingual=False, **kwargs):
        self.is_seen = kwargs.get("is_seen", True)
        
        if self.is_seen:
            self.name = "seen_discrete_units"
        else:
            self.name = "unseen_discrete_units"
        ...
        if self.is_multispkr and spk_emb_scp_filepath:
             with open(spk_emb_scp_filepath, "r") as f:
                for line in f:
                    ...
                    if len(parts) >= 2:
                        utt_id = parts[0].split("_",1)[-1] # Bengali_1407374883553505_chunk_6_enhanced -> 1407374883553505_chunk_6_enhanced
                        self.spk_map[utt_id] = parts[1]
        import json
        if is_multilingual and lang_ids_json and utt2lang_filepath:
            with open(lang_ids_json, "r") as f:
                self.lang_ids = json.load(f)
            ...
            with open(utt2lang_filepath, "r") as f:
                for line in f:
                    parts = line.strip().split()
                    if len(parts) >= 2:
                        self.utt2lang[parts[0]] = parts[1] # we need to make sure that utt ids are consistent across all files :-)
        

        # 2. Load Units (Strict 1:1 alignment with audio_files expected)
        with open(self.unit_file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
            self.unit_arr = [
                np.array([int(k) for k in line.split()[1:]], dtype=np.int32)
                for line in lines
            ]

    def _get_f0(self, filename):
        # Adjust extension if needed
        f0_path = filename.replace(".wav", ".f0.npy")
    ...
    def __getitem__(self, index):
        try:
            ...
            if not self.split:
                ...
                if self.is_multilingual: ret["lang_ids"] = lang_id
            ...
            #TRAINING (Random Crop)
                ...
                if self.is_multispkr: ret["spkr"] = spk_emb
                if self.is_multilingual: ret["lang_id"] = lang_id
