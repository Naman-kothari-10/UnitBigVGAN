...
from CodeBigvgan import CodeGenerator, LIDClassifier
...
def train(rank, a, h):
    ...
    if h.is_discrete_input: 
        generator = CodeGenerator(h).to(device)
    else:
        generator = BigVGAN(h).to(device)
    
    if h.lang_classification:
        lidclassifier = LIDClassifier(in_channels=h.num_mels,hidden_channels=128, num_languages=h.n_langs).to(device)
    ...
    if cp_g is None or cp_do is None:
        ...
        if h.lang_classification and "lidclassifier" in state_dict_do:
            lidclassifier.load_state_dict(state_dict_do["lidclassifier"])
    ...
    if h.num_gpus > 1:
        ...
        if h.lang_classification:
            lidclassifier = DistributedDataParallel(lidclassifier, device_ids=[rank]).to(device)
        mpd = DistributedDataParallel(mpd, device_ids=[rank]).to(device)
        mrd = DistributedDataParallel(mrd, device_ids=[rank]).to(device)
        # if h.lang_classification:
        #     g_params = (
        #         list(generator.parameters()) +
        #         list(lidclassifier.parameters())
        #     )
        # else:
    ...
    optim_g = torch.optim.AdamW(
        g_params, h.learning_rate, betas=[h.adam_b1, h.adam_b2]
    )
    ...
    if h.lang_classification:
        optim_lid = torch.optim.AdamW(
            lidclassifier.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2]
        )

    if state_dict_do is not None:
        optim_g.load_state_dict(state_dict_do["optim_g"])
        optim_d.load_state_dict(state_dict_do["optim_d"])
        if h.lang_classification and "optim_lid" in state_dict_do:
            optim_lid.load_state_dict(state_dict_do["optim_lid"])
    ...
    if h.lang_classification:
        scheduler_lid = torch.optim.lr_scheduler.ExponentialLR(optim_lid, gamma=h.lr_decay, last_epoch=last_epoch)
        
    if h.is_multispkr:
        print("Training With speaker embeddings.........")
    if h.lang_classification:
        print("Training With language classification loss .........")
    if h.is_multilingual:
        print("Training with Languauge embeddings...")
    ...
    if h.is_discrete_input:
        trainset = CodeDataset(
            audio_files=training_filelist,
            unit_file_path=a.training_unit_file,
            utt2lang_filepath = a.training_utt2lang_path,
            lang_ids_json = a.lang_ids_json_path,
            spk_emb_scp_filepath=a.ecapa_tdnn_spk_emb_scp_file, # contains the spk emb of all the splits
            segment_size=h.segment_size,
            unit_hop_size=h.unit_hop_size,
            num_kmeans_units=h.num_kmeans_units,
            is_multispkr = h.is_multispkr,
            is_multilingual = h.is_multilingual,
            n_fft=h.n_fft,
            num_mels=h.num_mels,
            hop_size=h.hop_size,
            win_size=h.win_size,
    ...
    if rank == 0: # this means that validation only occurs in first rank (first gpu card)
        if h.is_discrete_input:
            validset = CodeDataset(
            audio_files=validation_filelist,
            unit_file_path=a.validation_unit_file,
            utt2lang_filepath = a.validation_utt2lang_path,
            lang_ids_json = a.lang_ids_json_path,
            spk_emb_scp_filepath=a.ecapa_tdnn_spk_emb_scp_file,
            segment_size=h.segment_size,
            unit_hop_size=h.unit_hop_size,
            num_kmeans_units=h.num_kmeans_units,
            is_multispkr = h.is_multispkr,
        ...
        if not a.skip_unseen:
            for i in range(len(list_unseen_validation_filelist)):
                if h.is_discrete_input:
                    unseen_validset = CodeDataset(
                    audio_files=list_unseen_validation_filelist[i],
                    unit_file_path=a.unseen_validation_unit_file,
                    utt2lang_filepath = a.unseen_validation_utt2lang_path,
                    lang_ids_json = a.lang_ids_json_path,
                    spk_emb_scp_filepath=a.ecapa_tdnn_spk_emb_scp_file, 
                    segment_size=h.segment_size,
                    unit_hop_size=h.unit_hop_size,
        ...
        with torch.no_grad():
            ...
            for j, batch in enumerate(tqdm(loader)):
                if h.is_discrete_input:
                    ...
                    y_g_hat = generator(**batch)
                    
                else: # when mel is input
                    x, y, _, y_mel = batch
                    y = y.to(device)
                ...
                if (
                    ...
                    # Resample to 16000 for pesq
                        ...
                        problem_log = os.path.join(a.checkpoint_path, "pesq_bad_files.txt")

                        # extract filename safely
                        fname = filenames[0] if isinstance(filenames, (list, tuple)) else filenames
                        # append to a simple text file
                        with open(problem_log, "a", encoding="utf8") as f:
                            f.write(f"{fname}\n")

                        print("\nPESQ ERROR: skipping file")
                        print(f"File: {fname}")
                        print(f"Error: {e}\n")
                ...
                if j % a.eval_subsample == 0:  # Subsample every nth from validation set
                    ...
                    if not h.is_discrete_input:
                        ...
                        # use mel target as ground truth spec
                        spec_gt = y_mel[0].cpu()
                        
                    spec_delta = torch.clamp(
                        torch.abs(spec_gt - y_hat_spec.squeeze(0).cpu()),
                        min=1e-6,
                        max=1.0,
                    )
                    sw.add_figure(
    ...
    if h.lang_classification:
        criterion_lang = torch.nn.CrossEntropyLoss()
        lidclassifier.train()    
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if h.lang_classification:
                ...
                # Train on REAL audio (y)
                # _, lang_logits_real = lidclassifier(y)
                _, lang_logits_real = lidclassifier(y_mel) 
                loss_lid_aux = criterion_lang(lang_logits_real, lang_ids)
                loss_lid_aux.backward()
                optim_lid.step()
            ...
            if h.get("use_multiscale_melloss", False):  # uses wav <y, y_g_hat> for loss
                # print(f"{y.shape=},{y_g_hat.shape=} ")
                min_t = min(y.size(-1), y_g_hat.size(-1))
                y_aligned = y[..., :min_t]
                y_g_hat_aligned = y_g_hat[..., :min_t]

                loss_mel = fn_mel_loss_multiscale(y_aligned, y_g_hat_aligned) * lambda_melloss
            ...
            lang_loss = 0
            if h.lang_classification:
                # Pass generated audio to the still training classifier
                # _, lang_logits_gen = lidclassifier(y_g_hat)
                _,lang_logits_gen = lidclassifier(y_g_hat_mel)
                lang_loss = criterion_lang(lang_logits_gen, lang_ids)
            
            if steps >= a.freeze_step:
                
                if h.lang_classification:
                    lambda_lid = h.get("lambda_lid", 0.1)
                    loss_gen_all = (
                        loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel + (lambda_lid*lang_loss) # should i have lambda for lang loss?
                    )
                else:
                    loss_gen_all = (
            ...
            if rank == 0:
                ...
                if steps % a.checkpoint_interval == 0 and steps != 0:
                    ...
                    if h.lang_classification:
                        state_dict_do["lidclassifier"] = (lidclassifier.module if h.num_gpus > 1 else lidclassifier).state_dict()
                        state_dict_do["optim_lid"] = optim_lid.state_dict()
                        
                    save_checkpoint(checkpoint_path, state_dict_do)
                ...
                if steps % a.summary_interval == 0:
                    ...
                    if h.lang_classification:
                        sw.add_scalar("training/lang_loss", lang_loss.item(), steps)
                ...
                # Validation
                if steps % a.validation_interval == 0 and not h.is_discrete_input:
...
def main():
    ...
    parser.add_argument("--training_unit_file", default = "exp/bigvgan")
    parser.add_argument("--validation_unit_file", default = "exp/bigvgan")
    parser.add_argument("--unseen_validation_unit_file", default = "exp/bigvgan")
    parser.add_argument("--ecapa_tdnn_spk_emb_scp_file", default=None)
    parser.add_argument("--training_utt2lang_path", default=None)
    parser.add_argument("--validation_utt2lang_path", default=None)
    parser.add_argument("--unseen_validation_utt2lang_path", default=None)
    parser.add_argument("--lang_ids_json_path", default = None)
